...
{
	LCD_CS_LOW();
	GPIOB -> ODR = (GPIOB -> ODR & 0x1807) | (((data>>8) & 0x00ff)<<3);
	LCD_WR_STROBE();
	GPIOB -> ODR = (GPIOB -> ODR & 0x1807) | ((data & 0x00ff)<<3);
	LCD_WR_STROBE();
	LCD_CS_HIGH();
...
void LCD_Begin(void)
{
	...
	HAL_Delay(5);
		...
		LCD_WriteCMD(0x01);		//  Driver Output Control
		LCD_Write16(0x0100);
		LCD_WriteCMD(0x02);		//  LCD Driving Wave Control
		LCD_Write16(0x0700);
		LCD_WriteCMD(0x03);		//  Entry Mode ORIENTATION
		LCD_Write16(0x1008);
		LCD_WriteCMD(0x04);
		LCD_Write16(0x0000);
		LCD_WriteCMD(0x08);
		...
		LCD_WriteCMD(0x13);
		LCD_Write16(0x0000);
		LCD_WriteCMD(0x07);		//   Display Control 1
		LCD_Write16(0x0001);
		HAL_Delay(5);
...
{
  LCD_setAddrWindow (0,0,Width, Height);
  LCD_WriteCMD(0x22); //write memory start
  unsigned long index = ((Height+1) * (Width+1));
  while(index){
	  LCD_Write16(color);
	  index--;
  }
...
{
	LCD_setAddrWindow(Width-y1, x0, Width-y0, x1);
	LCD_WriteCMD(0x22); //write memory start

	uint16_t index = (y1-y0)*(x1-x0)*6;
	while(index)
	{
		LCD_Write16(color);
		index--;
